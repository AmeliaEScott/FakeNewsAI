...
connectionPool = pool.ThreadedConnectionPool(5, 50, database=config["database"], host=config["host"],
...
class NewsSpider(scrapy.Spider):
    ...
    def removewww(self, url):
        return re.sub(r'^((?:https?://)?)(w[w0-9][w0-9]\.)?(.*?)$', r'\1\3', url, flags=re.IGNORECASE)
        ...
        with ConnectionWrapper(connectionPool) as connection:
            with connection.cursor() as cursor:
                ...
                for link in links:
                    ...
                    if alreadyvisited and currentdomain == newuri.netloc:
