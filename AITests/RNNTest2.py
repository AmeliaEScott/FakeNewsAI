...
def buildtower(batchsize):
    ...
    return networkinput, initial_state, initial_hidden_state, loss, loss_mask, network_outputs
...
def buildgraph():
    ...
    with tf.variable_scope(VARIABLE_SCOPE), tf.device("/cpu:0"):
    ...
    for gpu in gpus:
        ...
        with tf.device(gpu):
            networkinput, initial_state, initial_hidden_state, loss, lossmask, networkoutputs_ = buildtower(BATCH_SIZE / len(gpus))
            networkinputs.append(networkinput)
            initial_states.append(initial_state)
            initial_hidden_states.append(initial_hidden_state)
            losses.append(loss)
            lossmasks.append(lossmask)
            gradients.append(optimizer.compute_gradients(loss))
            networkoutputs.append(networkoutputs_)
    ...
    with tf.device("/cpu:0"):
        ...
        allnetworkinputs = tf.concat(networkinputs, axis=0)
        allinitialstates = tf.concat(initial_states, axis=0)
        allinitialhiddenstates = tf.concat(initial_hidden_states, axis=0)
        averagelosses = sum(losses) / len(losses)
        alllossmasks = tf.concat(lossmasks, axis=0)
        alloutputs = tf.concat(networkoutputs, axis=0)

    return allnetworkinputs, alloutputs, allinitialstates, allinitialhiddenstates, \
        averagelosses, train_step, alllossmasks
